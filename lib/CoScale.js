...
doHttpRequest = function (method, apiPath, coscaleMessage, callback) {
    ...
    req = protocol.request(options, function (res) {
        if (debug) {
            util.log("statusCode: " + res.statusCode + ", coscaleMessage: " + JSON.stringify(coscaleMessage));
...
// addMetricGroups will create nested metricgroups. if parentGroupId is null first group will be a main group
var addMetricGroups = function (parentGroupId, groups, metricId) {
    // pull out the first group from groups array
    var group = groups.shift(),
        callback;
    // set the callback
    if (groups.length === 0) {
        // if this was the last group, the callback should add the metric into this group
        callback = function(groupId){
            addMetricToGroup(groupId, metricId);
        };
    } else {
        // otherwise the callback should call again addMetricGroups for the rest of the subgroups
        callback = function(groupId){
            addMetricGroups(groupId, groups.slice(0), metricId);
        };
    }
    addMetricGroup(group, parentGroupId, callback);
}

// add a new group for metrics
var addMetricGroup = function (groupName, parentGroupId, callback) {
    ...
    var data = {
        ...
        // metricgroups uri for http request
        location = coscaleApiPath + coscaleAppId + '/metricgroups/';
    // if we have a parentGroupId add this new group as a subgroup
    if (parentGroupId) {
        data.parentId = parentGroupId;
    }
    doHttpRequest('POST', location, data, function (statusCode, res) {
...
var addMetric = function (metricName, dataType, callback) {
    ...
    doHttpRequest('POST', location, data, function (statusCode, res) {
        var metricId = res.id;
        if (statusCode === 200 || res.type === 'DUPLICATE') {
            addMetricGroups(null, getMetricGroups(metricName), metricId);
        }
...
// TODO: we need a format to include the hostname
...
// getMetricGroups is used to get the groups names from a metric name. The output will be an array of strings
var getMetricGroups = function(metricName) {
    groups = metricName.split('.');
    // add the default 'StatsD' group as the main group
    groups.unshift(metricgroup);
    // remove the last element which should not be a subgroup
    groups.pop();
    return groups;
};
